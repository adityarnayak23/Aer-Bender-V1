...
// --- 2. Verify Audio Synthesis Bass & Weight Enhancements + Highs Reduction ---
console.log('\n--- 2. Testing Audio Synthesis Bass & Weight Enhancements + Highs Reduction ---');

// Sub-Harmonic oscillator boost for mid (0.66) & higher (0.58) notes
assert(audioContent.includes('oscMixGainSub.gain.setValueAtTime(isLowerOctave ? 0.82 : (isTaraOctave ? 0.58 : 0.66), now)'),
  'Electric voice must have strong sub-harmonic oscillator gain for mid (0.66) and higher (0.58) notes');
console.log('✅ PASS: Sub-harmonic triangle oscillator boost active for mid (0.66) & higher (0.58) notes');

// Low-shelf bass boost (bassEQ) in electric voice
assert(audioContent.includes('bassEQ.gain.setValueAtTime(10.2, now)'),
  'Electric voice must provide +10.2dB low shelf boost for mid notes');
assert(audioContent.includes('bassEQ.gain.setValueAtTime(9.0, now)'),
  'Electric voice must provide +9.0dB low shelf boost for higher notes');
console.log('✅ PASS: Resonant low-shelf bass filter active for mid (+10.2dB) and high (+9.0dB) notes');
...
assert(audioContent.includes('bodyWeightEQ.gain.setValueAtTime(isLowerOctave ? 5.0 : 6.8, now)'),
  'Body weight filter must provide +6.8dB warmth boost for mid and higher notes');
console.log('✅ PASS: Wood & bore acoustic body weight filter (+6.8dB) active in electric voice');

// High Cut EQ in electric voice
assert(audioContent.includes('highCutEQ'), 'Electric voice must include highCutEQ filter');
assert(audioContent.includes('highCutEQ.gain.setValueAtTime(-7.5, now)'),
  'High cut filter must provide -7.5dB reduction to tame harsh treble glare');
console.log('✅ PASS: High frequency cut filter (-7.5dB) active to reduce highs');

// Warm lowpass filter cutoff ceiling
assert(audioContent.includes('Math.min(3600, Math.max(1000, targetFreq * 2.2))'),
  'Lowpass filter ceiling must be capped at 3600Hz to eliminate piercing highs');
console.log('✅ PASS: Lowpass filter cutoff ceiling capped at 3600Hz');

// Legato transitions maintain bass, body weight, and tamed highs
assert(audioContent.includes('targetBassGain = isLowerOctave ? 11.5 : (isTaraOctave ? 9.0 : 10.2)'),
  'Legato transitions must preserve bass boost across octaves');
assert(audioContent.includes('targetSubGain = isLowerOctave ? 0.82 : (isTaraOctave ? 0.58 : 0.66)'),
...
// Acoustic sample voice (playSwara) body bass and high cut
assert(audioContent.includes('bodyBassFilter.gain.setValueAtTime(9.2, now)'),
  'Sample voice must provide +9.2dB bass weight for mid notes');
assert(audioContent.includes('bodyBassFilter.gain.setValueAtTime(8.0, now)'),
  'Sample voice must provide +8.0dB bass weight for higher notes');
assert(audioContent.includes('highCutFilter.gain.setValueAtTime(-6.0, now)'),
  'Sample voice must provide -6.0dB high cut filter');
console.log('✅ PASS: Acoustic sample voice equips resonant bass filter (+9.2dB/+8.0dB) and high cut (-6.0dB)');

// --- 2b. Verify Eye Glow Completely Removed ---
console.log('\n--- 2b. Testing Eye Glow Removal ---');
assert(!trackerContent.includes('this.renderEyeballOctaveGlow(ctx, width, height, scale, toScreen, now)'),
  'renderCleanOverlay must NOT invoke renderEyeballOctaveGlow');
console.log('✅ PASS: Eye glow call removed from overlay rendering');
